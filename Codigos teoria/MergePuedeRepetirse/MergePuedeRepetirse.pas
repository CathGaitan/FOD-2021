...
begin
    ...
    while (min.codigo <> valoralto) do begin
        codprod:=min.codigo; //me guardo mi codigo minimo
        canttotal:=0; //inicializo mi contador
        while (codprod=min.codigo) do begin //mientras el codigo sea el mismo
            canttotal:=canttotal+min.cant; //cuento cantidad de productos
            minimo(regD1,regD2,regD3,min); //busco otro minimo
        end;
        write(mae1,min); //actualizo mi MAESTRO
    end;
    close(det1);
    close(det2);
    close(det3);
